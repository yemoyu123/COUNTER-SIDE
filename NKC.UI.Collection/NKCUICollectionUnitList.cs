...
public class NKCUICollectionUnitList : MonoBehaviour
{
	...
	public static List<NKMUnitData> UpdateCollectionUnitList(ref int collectionUnit, ref int totalUnit, NKM_UNIT_TYPE eUnitType, bool getUnitDataList)
	{
		...
		for (int i = 0; i < unitList.Count; i++)
		{
			...
			if ((unitTemplet == null || !unitTemplet.m_bExclude || IsHasUnit(eUnitType, unitTempletBase.m_UnitID)) && unitTempletBase.PickupEnableByTag)
			{
				if (getUnitDataList)
				{
					list.Add(NKCUtil.MakeDummyUnit(unitList[i], 100, 3));
				}
				if (IsHasUnit(eUnitType, unitList[i]))
				{
					collectionUnit++;
				}
				totalUnit++;
	...
	public void Open()
	{
		...
		m_iCollectionUnit = 0;
		m_ssActive = null;
		NKCUtil.SetGameobjectActive(m_tglShowHasReward, bValue: false);
		...
		if (eUnitType == NKM_UNIT_TYPE.NUT_NORMAL || eUnitType != NKM_UNIT_TYPE.NUT_SHIP)
		{
			...
			NKCUtil.SetGameobjectActive(m_tglShowHasReward, bValue);
	...
	private void ProvideSlotData(Transform tr, int idx)
	{
		...
		component.SetDataForCollection(nKMUnitData, deckIndexCache, OnSlotSelected, IsHasUnit(m_eUnitType, nKMUnitData.m_UnitID));
	...
	public void CheckRewardToggle()
	{
		if (!(m_tglShowHasReward == null))
		{
			...
			NKCUtil.SetGameobjectActive(m_tglShowHasReward, flag);
