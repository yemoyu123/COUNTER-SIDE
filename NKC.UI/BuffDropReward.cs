...
public class BuffDropReward : StageRewardData
{
	private int m_iRewardGroupId;

	public BuffDropReward(Transform slotParent)
		: base(slotParent)
	{
	}
	...
	{
		if (stageTemplet == null || !NKMEpisodeMgr.CheckStageHasBuffDrop(stageTemplet))
		{
			return;
		}
		m_iRewardGroupId = stageTemplet.GetBuffDropRewardGroupID();
		...
		if (count2 > 1)
		{
			...
			m_cSlot.SetData(data, bShowName: false, bShowNumber: false, bEnableLayoutElement: true, OnClickBuffDropSlot);
			m_cSlot.SetBackGround(num);
			m_cSlot.SetBuffDropMark(bValue: true);
	...
	private void OnClickBuffDropSlot(NKCUISlot.SlotData slotData, bool bLocked)
	{
		NKMRewardGroupTemplet rewardGroup = NKMRewardManager.GetRewardGroup(m_iRewardGroupId);
		if (rewardGroup == null)
		{
