...
public class NKCUIGauntletLobbyRank : MonoBehaviour
{
	...
	private void UpdateRemainTimeUI()
	{
		...
		{
			if (!pvpRankSeasonTemplet.CheckMySeason(NKCSynchronizedTime.GetServerUTCTime()))
			{
				m_lbRemainRankTime.text = NKCUtilString.GET_STRING_GAUNTLET_THIS_SEASON_LEAGUE_BEING_EVALUATED;
			}
			else
			{
				m_lbRemainRankTime.text = string.Format(NKCUtilString.GET_STRING_GAUNTLET_THIS_SEASON_LEAGUE_REMAIN_TIME_ONE_PARAM, NKCUtilString.GetRemainTimeStringEx(pvpRankSeasonTemplet.EndDate));
			}
		}
		else
		{
	...
	private void Update()
	{
		if (m_fPrevUpdateTime + 1f < Time.time)
		{
			...
			UpdateRemainTimeUI();
			m_RightSide.UpdateReadyRankButtonUI();
	...
	public void ProvideDataMyLeague(Transform tr, int index)
	{
		...
		if (component != null)
		{
			component.SetUI(GetUserSimpleData(RANK_TYPE.MY_LEAGUE, index), index + 1, NKM_GAME_TYPE.NGT_PVP_RANK);
		}
	}
	...
	public void ProvideDataAll(Transform tr, int index)
	{
		...
		if (component != null)
		{
			component.SetUI(GetUserSimpleData(RANK_TYPE.ALL, index), index + 1, NKM_GAME_TYPE.NGT_PVP_RANK);
		}
	}
	...
	public void ProvideDataFriend(Transform tr, int index)
	{
		...
		if (component != null)
		{
			component.SetUI(GetUserSimpleData(RANK_TYPE.FRIEND, index), index + 1, NKM_GAME_TYPE.NGT_PVP_RANK);
		}
	}
	...
	{
		m_RightSide.UpdateNowSeasonPVPInfoUI(NKM_GAME_TYPE.NGT_PVP_RANK);
	...
	{
		AddUserSimpleList(cNKMPacket_PVP_RANK_LIST_ACK.rankType, cNKMPacket_PVP_RANK_LIST_ACK.userProfileDataList);
		if (cNKMPacket_PVP_RANK_LIST_ACK.rankType == RANK_TYPE.FRIEND)
		{
			GetUserSimpleList(cNKMPacket_PVP_RANK_LIST_ACK.rankType).Sort((NKMUserSimpleProfileData a, NKMUserSimpleProfileData b) => b.pvpScore.CompareTo(a.pvpScore));
		}
		if (cNKMPacket_PVP_RANK_LIST_ACK.userProfileDataList.Count < NKMPvpCommonConst.Instance.RANK_SIMPLE_COUNT)
		{
			m_arAllRankREQ[(int)cNKMPacket_PVP_RANK_LIST_ACK.rankType] = true;
		}
		if (m_RANK_TYPE == cNKMPacket_PVP_RANK_LIST_ACK.rankType)
		{
			RefreshRankTabCells();
		}
	}
	...
	{
		m_RightSide.UpdateCastingBanVoteState();
	}
	...
	public void SetUI()
	{
		...
		m_RightSide.UpdateReadyRankButtonUI();
		m_RightSide.UpdateRankPVPPointUI();
		m_RightSide.UpdateRankBotMatchUI();
		SetRankTabUI();
		TutorialCheck();
	}
	...
	private void SendPVPListREQ(RANK_TYPE type, bool all)
	{
		...
		nKMPacket_PVP_RANK_LIST_REQ.rankType = m_RANK_TYPE;
		nKMPacket_PVP_RANK_LIST_REQ.isAll = all;
		NKCScenManager.GetScenManager().GetConnectGame().Send(nKMPacket_PVP_RANK_LIST_REQ, NKC_OPEN_WAIT_BOX_TYPE.NOWBT_INVALID);
	}
	...
	{
		NKCTutorialManager.TutorialRequired(TutorialPoint.GauntletLobbyRank);
	}
}
