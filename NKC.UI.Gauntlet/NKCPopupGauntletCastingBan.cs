...
public class NKCPopupGauntletCastingBan : NKCUIBase
{
	...
	public static NKCPopupGauntletCastingBan Instance
	{
		get
		{
			if (m_Instance == null)
			{
				m_Instance = NKCUIManager.OpenNewInstance<NKCPopupGauntletCastingBan>("AB_UI_NKM_UI_GAUNTLET", "NKM_UI_GAUNTLET_POPUP_BANNED_LIST_CASTING", NKCUIManager.eUIBaseRect.UIFrontPopup, CleanupInstance).GetInstance<NKCPopupGauntletCastingBan>();
				m_Instance.InitUI();
			}
			return m_Instance;
		}
	...
	public void InitUI()
	{
		...
		NKCUtil.SetGameobjectActive(m_ctglOper.gameObject, NKCOperatorUtil.IsActiveCastingBan());
	...
	public void Open()
	{
		...
		m_bCheckCastingBanRemainTime = true;
		UpdateUI();
		UIOpened();
	}
	...
	public void UpdateUI()
	{
		...
		if (m_SelectTabType == NKM_UNIT_TYPE.NUT_NORMAL)
		{
			...
			m_lstCastingVotedUnits = NKCBanManager.m_CastingVoteData.unitIdList.ToList().FindAll((int e) => e > 0);
		...
		else if (m_SelectTabType == NKM_UNIT_TYPE.NUT_SHIP)
		{
			...
			foreach (int ShipGroupId in NKCBanManager.m_CastingVoteData.shipGroupIdList)
			{
		...
		{
			m_lstCastingVotedUnits.Clear();
			m_lstCastingVotedUnits = NKCBanManager.m_CastingVoteData.operatorIdList.ToList().FindAll((int e) => e > 0);
			flag = m_lstCastingVotedUnits.Count > 0;
			if (flag)
			{
				m_lstCastingVotedUnits.Sort();
	...
	private void OnClickSelectList()
	{
		...
		foreach (int item in unitList)
		{
			...
			if (unitTempletBase != null && unitTempletBase.PickupEnableByTag && (m_SelectTabType != NKM_UNIT_TYPE.NUT_NORMAL || !NKCBanManager.IsBanUnit(item) || NKCBanManager.GetUnitBanLevel(item) < 2) && (m_SelectTabType != NKM_UNIT_TYPE.NUT_SHIP || !NKCBanManager.IsBanShip(unitTempletBase.m_ShipGroupID) || NKCBanManager.GetShipBanLevel(unitTempletBase.m_ShipGroupID) < 2) && (m_SelectTabType != NKM_UNIT_TYPE.NUT_OPERATOR || !NKCBanManager.IsBanOperator(item) || NKCBanManager.GetOperBanLevel(item) < 2))
			{
