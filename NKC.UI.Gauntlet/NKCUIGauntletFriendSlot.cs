...
public class NKCUIGauntletFriendSlot : MonoBehaviour
{
	...
	public static NKCUIGauntletFriendSlot GetNewInstance(Transform parent, OnDragBegin onDragBegin)
	{
		NKCAssetInstanceData nKCAssetInstanceData = NKCAssetResourceManager.OpenInstance<GameObject>("AB_UI_NKM_UI_GAUNTLET", "NKM_UI_GAUNTLET_FRIEND_SLOT");
	...
	{
		if (NKCPrivatePVPRoomMgr.IsDraftBanMode && !NKCBanManager.IsTryDraftBan())
		{
			NKCPopupMessageManager.AddPopupMessage(NKCUtilString.GET_STRING_GAUNTLET_GLOBAL_BAN_NOT_SELECT_UNIT);
		}
		else
		{
			NKCPrivatePVPRoomMgr.Send_NKMPacket_PRIVATE_PVP_LOBBY_CREATE_REQ(m_FriendListData);
		}
	}
}
