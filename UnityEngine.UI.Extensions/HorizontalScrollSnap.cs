...
public class HorizontalScrollSnap : ScrollSnapBase
{
	...
	public override void OnEndDrag(PointerEventData eventData)
	{
		...
		if (UseHardSwipe)
		{
			...
			{
				if (_startPosition.x - _screensContainer.anchoredPosition.x > 0f)
				{
					NextScreen();
				}
				else
		...
		else
		{
			...
			if (_startPosition.x - _screensContainer.anchoredPosition.x > 0f)
			{
				if (_startPosition.x - _screensContainer.anchoredPosition.x > _childSize / 3f)
				{
					ScrollToClosestElement();
				}
				else
			...
			else if (_startPosition.x - _screensContainer.anchoredPosition.x < (0f - _childSize) / 3f)
			{
				ScrollToClosestElement();
			}
			else
